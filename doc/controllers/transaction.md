...
```php
function pricedTransactions(string $requestId, ?PriceTransactionReq $body = null): PricedTransactionRes
```
...
$body = PriceTransactionReqBuilder::init()
    ->page(1)
    ->pageSize(100)
    ->build();
...
$body = MultiPricedTransactionReqBuilder::init()
    ->page(1)
    ->pageSize(100)
    ->build();
...
```php
function cardUsageSummary(string $requestId, ?CardUsageSummaryReq $body = null): CardUsageSummaryRes
```
...
```php
function volumeBasedBonus(string $requestId, ?VolumeBasedBonusReq $body = null): VolumeBasedBonusRes
```
...
```php
function volumeBasedPricing(string $requestId, ?VolumeBasedPricingReq $body = null): VolumeBasedPricingRes
```
...
```php
function fees(string $requestId, ?TransactionFeesReq $body = null): TransactionFeesRes
```
...
$body = TransactionFeesReqBuilder::init()
    ->page(1)
    ->pageSize(100)
    ->build();
...
```php
function feeSummaryResponse(string $requestId, ?TransactionFeesSummaryReq $body = null): FeeSummaryResponse
```
...
```php
function fuelConsumption(string $requestId, ?FuelConsumptionReq $body = null): FuelConsumptionResponse
```
...
```php
function updateOdometer(string $requestId, ?UpdateOdometerRequest $body = null): UpdateOdometerResp
```
