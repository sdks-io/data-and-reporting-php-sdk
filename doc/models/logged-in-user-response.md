...
| `isSuperAdmin` | `?bool` | Optional | Whether the user is a super administrator<br><br>**Default**: `false` | getIsSuperAdmin(): ?bool | setIsSuperAdmin(?bool isSuperAdmin): void |
...
| `displayWeek` | `?bool` | Optional | Preferred Display Week on Fuel Prices configuration for the logged in User.<br><br>**Default**: `true` | getDisplayWeek(): ?bool | setDisplayWeek(?bool displayWeek): void |
...
| `hasAPIAccess` | `?bool` | Optional | True/False<br>True, if user has access to the requested API. This is validated based on the role Vs API access matrix and client Vs API access matrix.<br><br>**Default**: `true` | getHasAPIAccess(): ?bool | setHasAPIAccess(?bool hasAPIAccess): void |
